...
namespace ProjectTracker.Api.Features.Projects.GetProject
{
    public class GetProjectFeature : IEndpoint
    {
        ...
        private static async Task<IResult> Handle(int id, IProjectService projectService, ICurrentUser currentUser)
        {
            ...
            return Results.Ok(new GetProjectResponse(project.Id, project.Name, project.Description, project.CreatedAt, tasks));
        }
    }
    ...
    public record GetProjectResponse(int Id, string Name, string? Description, DateTime CreatedAt, IEnumerable<TaskDetail> Tasks);
}
